...
def get_tangent_direction(
    ...
    # project to xy plane
    estimated_pivot_point[2] = 0
    cur_p[2] = 0
    arrow = estimated_pivot_point - cur_p
    ...
    if estimated_axis_direction[2] < 0:
        tangent_direction = -tangent_direction

    return tangent_direction

def batch_get_tangent_direction(
    batch_estimated_axis_direction, 
    batch_estimated_pivot_point, 
    batch_cur_p, 
):
    ret = []

    for estimated_axis_direction, estimated_pivot_point, cur_p \
        in zip(batch_estimated_axis_direction, batch_estimated_pivot_point, batch_cur_p):

        ret.append(
            get_tangent_direction(estimated_axis_direction, estimated_pivot_point, cur_p)
        )
...
def batch_get_next_direction_dot(
    cur_dir_list, target_dir_list, 
    t = 2
):
    delta_list = target_dir_list - cur_dir_list

    dot_list = (target_dir_list * cur_dir_list).sum(
        axis = -1, 
        keepdims = True
    )
    dot_list = np.clip(
    ...
    next_dir_list = normalize(
        cur_dir_list + t * delta_list * dot_list
    )

    next_dir_list = np.asarray(next_dir_list)
    next_dir_list[:, 2] = 0  # project to xy plane
