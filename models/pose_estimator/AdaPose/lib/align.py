...
def estimatePnPRansac(nocsPts, cameraPts, size, intrinsic):
    temp_nocsPts = nocsPts * size
    # _, r, t, _ = cv2.solvePnPRansac(temp_nocsPts, cameraPts, intrinsic, np.zeros(4), flags=cv2.SOLVEPNP_EPNP)
    success, r, t, _ = cv2.solvePnPRansac(temp_nocsPts, cameraPts, intrinsic, np.zeros(4), flags=cv2.SOLVEPNP_EPNP, reprojectionError = 3.0)
    if success:
        criteria = (cv2.TERM_CRITERIA_MAX_ITER+cv2.TERM_CRITERIA_EPS, 20, 1e-6)
        r, t = cv2.solvePnPRefineVVS(temp_nocsPts, cameraPts, intrinsic, None, r, t, criteria=criteria)
    R, _ = cv2.Rodrigues(r)
    sRT = np.eye(4).astype(np.float32)
    sRT[:3, :3] = R * size
    sRT[:3, 3] = t.flatten()
    return success, size, R, t, sRT
