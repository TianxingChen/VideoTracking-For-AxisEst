...
class PickMugManipulation(BaseManipulation) :
    ...
    def plan_pathway(self, center, axis, eval=False) :
        ...
        res = self.env.hand_move_to(pre_grasp_pose, time=2, wait=2, planner="path", no_collision_with_front=False)

        grasp_p = pre_grasp_p + pre_grasp_axis * 0.06
        grasp_q = pre_grasp_q
        grasp_pose = np.concatenate([grasp_p, grasp_q], axis=-1)
        res = self.env.hand_move_to(grasp_pose, time=2, wait=1, planner="ik" if self.cfg["closed_loop"] else "path")
        ...
        for step_size in self.cfg["step_sizes"] :
            ...
            self.env.gripper_move_to(pred_pose, time=step_size*10, wait=step_size*5, planner="ik" if self.cfg["closed_loop"] else "path")
