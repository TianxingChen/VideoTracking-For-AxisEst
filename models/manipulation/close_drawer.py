...
class CloseDrawerManipulation(BaseManipulation) :
    ...
    def plan_pathway(self, center, axis, eval=False) :
        ...
        # batch_size = obs["handle_bbox"].shape[0]
        ...
        for step_size in self.cfg["step_sizes"] :
            ...
            # self.env.class_method("_draw_point", cur_p[0], color=[0,1/,0], ret=False)
            pred_p = cur_p + cur_dir * step_size

            next_y = -z_
            next_z = -cur_dir
            next_x = np.cross(next_y, next_z)
            # print((next_x*next_z).sum(), (next_y*next_z).sum(), (next_x*next_y).sum())
            axis_from = np.concatenate([
                x_[:, np.newaxis, :],
                y_[:, np.newaxis, :],
                z_[:, np.newaxis, :]
