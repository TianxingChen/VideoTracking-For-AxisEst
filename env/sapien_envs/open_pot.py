...
class OpenPotEnv(BaseManipulationEnv):
    ...
    def _generate_object_config(self) :
        ...
        def randomize_pose(ang_low, ang_high, rot_low, rot_high, dis_low, dis_high, height_low, height_high) :
            ...
            # if self.obj_cfg["type"] == "mug" :
            #     r90 = sapien.Pose(q=axis_angle_to_quat([1,0,0], np.pi/2))
            #     # rp = sapien.Pose(q=axis_angle_to_quat([0,0,1], np.pi/2))
            #     p1 = r0 * p0 * r1 * r90
            # else :
            p1 = r0 * p0 * r1
        ...
        pose = randomize_pose(
            ...
            self.obj_init_dis_low + 0.3,
            self.obj_init_dis_high + 0.3,
            self.obj_init_height_low + 0.3,
            self.obj_init_height_high + 0.3
        )
        dof = randomize_dof(
    ...
    def handle_pose(self) :
        ...
        handle_x = (handle_bbox[4] - handle_bbox[0])
        handle_y = (handle_bbox[3] - handle_bbox[0])
        handle_z = (handle_bbox[1] - handle_bbox[0])
    ...
    def get_reward(self, action) :
        ...
        return near_reward + open_reward * (dist < 0.1)
                ...
                if link_name == link.get_name():
                    ...
                    for k in part_v.keys():
                        if 'lid' in k:
                            ...
                            # self._clear_point()
                            # self._draw_point(global_bbox[0])
                            # self._draw_point(global_bbox[1])
                            # self._draw_point(global_bbox[2])
                            # self._draw_point(global_bbox[3])
                            # self._draw_point(global_bbox[4])
                            # self._draw_point(global_bbox[5])
                            # self._draw_point(global_bbox[6])
                            # self._draw_point(global_bbox[7])
