...
def test(env : MultiVecEnv, controller : BaseController, cfg : dict) :
    ...
    for i in range(total_round) :
        ...
        no_mask_flag = controller.run()
        num_no_mask += no_mask_flag

        obs = env.get_observation()
        # move_distance += np.sum(obs["total_move_distance"])
        success += np.sum(obs["success"])
        object_dof = obs["object_dof"][:, 0]
        total_num_traj += obs["success"].shape[0]

        print(obs["success"][:, 0])
        print(f"object_dof: {object_dof} ({np.degrees(object_dof)})")

        if (not no_mask_flag) or (no_mask_flag and (not cfg["train"]["skip_no_mask"])):
            env.save_obs_list()
            
        env.reset(save_video = cfg["train"]["save_video"])

    env.close()

    logger.log.info("Total round: %d" % total_num_traj)
    # logger.log.info("Success round: %d / %d" % success, total_num_traj)
    # logger.log.info("Success rate: %f" % (success / total_num_traj))
    # logger.log.info("No mask rate: %f" % (num_no_mask / total_num_traj))
