...
class StereoPoseNet_with_depth_baseline(nn.Module):
    def __init__(self, n_cat=1, nv_pts = 1024, img_arc = 'ResNet34', stereo_fusion = True, regress_pose = False):
        super(StereoPoseNet_with_depth_baseline, self).__init__()
        self.n_cat = n_cat
        self.img_arc = img_arc
        self.stereo_fusion = stereo_fusion
        self.regress_pose = regress_pose
        ...
        self.view_fusion = ViewFusion(embed_dim = 32, num_heads = 4, depth = 4)
    ...
    def forward(self, view1_img, view1_choose, view2_img, view2_choose, view1_proj, view2_proj, depth_values):
        ...
        view1_emb = view1_feat_img.view(bs, di, -1)
        view1_nocs_choose = view1_choose.unsqueeze(1).repeat(1, di, 1)
        view1_nocs_feat_t = torch.gather(view1_emb, 2, view1_nocs_choose).contiguous()
        view1_nocs_feat = self.instance_color(view1_nocs_feat_t)

        view2_emb = view2_feat_img.view(bs, di, -1)
        view2_nocs_choose = view2_choose.unsqueeze(1).repeat(1, di, 1)
        view2_nocs_feat_t = torch.gather(view2_emb, 2, view2_nocs_choose).contiguous()
        view2_nocs_feat = self.instance_color(view2_nocs_feat_t)

        view1_nocs = self.nocs_head(view1_nocs_feat)
        view2_nocs = self.nocs_head(view2_nocs_feat)

        fused_view1_features, fused_view2_features = self.view_fusion(view1_nocs_feat_t, view2_nocs_feat_t)

        view1_depth = self.depth_head(fused_view1_features).squeeze(1)
        view2_depth = self.depth_head(fused_view2_features).squeeze(1)
