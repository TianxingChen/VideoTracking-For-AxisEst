...
from utils.transform import normalize, axis_angle_to_quat
...
class BaseEnv:
    ...
    def get_viewer(self, camera_intrinsics, camera_extrinsics : Union[sapien.Pose, np.ndarray], mount=None):
        ...
        def get_param_method() :
            return camera.get_model_matrix(), \
                camera.get_intrinsic_matrix(), camera.get_extrinsic_matrix()
    ...
    def _draw_line(
        ...
        pose = sapien.Pose(
            p = st_point + direction * (length / 2),  # mid point
        ...
        line = actor_builder.build(name = name)
        line.set_pose(pose)
        line.lock_motion()
